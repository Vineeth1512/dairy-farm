...
const SingleProduct = ({ singleProduct, setCartCount }) => {
  const { id } = useParams();
  const cattle = singleProduct.find((c) => c.id === Number(id));
  ...
  return (
    <>
      <BreadCrumb
        title="Milk Products"
        breadcrumbs={[
          { label: "Milk Products", to: "/products" },
          { label: "Products" },
        ]}
      />
